...
public:
    ...
    */
    ...
    std::vector<float> bounds() const {
        float end_x = globalOrigin_.x + widthInMeters();
        float end_y = globalOrigin_.y + heightInMeters();
        std::vector<float> b = {globalOrigin_.x, end_x, globalOrigin_.y, end_y};
        return b;
    }

    bool isCellOccupied(const int x, const int y) const { return logOdds(x, y) < occupiedThreshold_; }
    ...
    */
    void fromLCM(const mbot_lcm_msgs::occupancy_grid_t& gridMessage);
